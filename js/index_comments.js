...
async function tfdisplayTweetForAnnotate(tweet_id_) {
  ...
  if (tweet_id_ === undefined) {
    ...
    const response = await fetch(
      proxy + "/users/assigned-tweets?thirdAnnotator=false",
      {
        method: "GET",
        headers: {
    ...
    if (response.status === 200) {
      ...
      if (
        data.data.tweets[i].tweet_id in data.data.tweets[i].finalAnnotation ===
        true
      ) {
        if (
          data.data.tweets[i].finalAnnotation[data.data.tweets[i].tweet_id] ===
          "HOF"
        ) {
        ...
        tab_1 +=
          `<li class="list-group-item d-flex justify-content-between align-items-center input-group-prepend list-group-item-primary list-group affix my-3 sticky-top">
                <span class="w-90" id="main_tweet">${data.data.tweets[i].tweet}</span><div class="btn-group" data-toggle="buttons">` +
          innerhtml +
          `</div></li>`;
      }
      for (comm_key in data.data.tweets[i].comments) {
        ...
        if (
          data.data.tweets[i].comments[comm_key].tweet_id in
            data.data.tweets[i].conflictedTweets ===
            true &&
          data.data.tweets[i].comments[comm_key].tweet_id in
            data.data.tweets[i].finalAnnotation ===
            false
        ) {
          ...
          if (
            data.data.tweets[i].comments[comm_key].tweet_id in
              data.data.tweets[i].annotations ===
            true
          ) {
            if (
              data.data.tweets[i].annotations[
                data.data.tweets[i].comments[comm_key].tweet_id
              ...
              tab_1 +=
                `<li class="list-group-item d-flex justify-content-between align-items-center input-group-prepend list-group-item-success">
                    <span class="w-90" id="main_tweet" >${data.data.tweets[i].comments[comm_key].tweet}</span><div class="btn-group" data-toggle="buttons">` +
                innerhtml +
                `</div></li> `;
            } else {
    ...
    if (total_tweet === 0) {
      ...
      if (Comment_activating_flag === true) {
        ...
        for (let i = 0; i < total_tweet; i++) {
          if (data.data.tweets[i].assignedTo.slice(0, 2).includes(name)) {
            if (
              data.data.tweets[i].tweet_id in
                data.data.tweets[i].finalAnnotation ===
              true
            ) {
              tab += `<tr>
        ...
        for (let i = 0; i < total_tweet; i++) {
          if (data.data.tweets[i].assignedTo.slice(2, 3).includes(name)) {
            ...
            for (let j = 0; j < data.data.tweets[i].comments.length; j++) {
              if (
                data.data.tweets[i].comments[j].tweet_id in
                  data.data.tweets[i].conflictedTweets ===
                  true &&
                data.data.tweets[i].comments.tweet_id in
                  data.data.tweets[i].finalAnnotation ===
                  false
              ) {
                tab += `<tr>
            ...
            for (let j = 0; j < data.data.tweets[i].comments.length; j++) {
              if (
                ...
                if (
                  Object.keys(
                    data.data.tweets[i].annotations[
                      data.data.tweets[i].comments[j].tweet_id
                    ]
                  ).length === 3
                ) {
                  if (
